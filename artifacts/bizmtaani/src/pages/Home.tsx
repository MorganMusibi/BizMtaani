...
import {
AreaPickerSheet,
} from "@/components/AreaPickerSheet";
...
// Support BOTH old string arrays
// and new object arrays.
const firstImage =
product.imageUrls?.[0];
...
const priceLabel =
isAccommodation
? "KES ${( product.rentPerMonth ?? product.price ).toLocaleString( "en-GB" )}/mo"
: isEatery
? null
: product.pricingBasis ===
"quote_only"
? "Quote only"
: product.price > 0
? "KES ${product.price.toLocaleString( "en-GB" )}${basisSuffix}${ negotiable ? " · Neg." : "" }"
: negotiable
? "Negotiable"
: null;
...
pos.coords.longitude,
  ...
  async () => {
    // GPS denied:
    // use saved home area or Nairobi.
    let coords: [
      number,
      number
    ] = NAIROBI;
...
const run =
  async () => {
    ...
    */
    ...
    /*
    |--------------------------------------------------------------------------
    | INITIAL NEARBY BUFFER LOAD
    |--------------------------------------------------------------------------
    */

    try {
      const nearby =
        await loadNearbyBuffer(
          userCoords,
          radiusKm
        );
      ...
      <div className="flex-shrink-0 bg-card border-t border-border px-4 py-3 flex items-center gap-3 z-40">
        <div className="flex-1 min-w-0">
          <p className="font-bold text-sm">
            Sell to buyers near you
          </p>

          <p className="text-xs text-muted-foreground">
            Sign in to post an advert
          </p>
        </div>
