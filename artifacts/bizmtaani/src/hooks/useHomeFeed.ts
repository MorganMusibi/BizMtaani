...
export function getProductVisibilityScope(
product: Product
) {
...
const scope =
getProductVisibilityScope(product);

// Free/local adverts
if (scope === "local") {
const radius =
product.visibilityRadiusKm ?? 2.5;
...
export function sortNearbyProducts(
products: Product[],
userCoords: [number, number]
): Product[] {
